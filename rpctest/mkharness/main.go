...
import (
	...
	"github.com/abcsuite/abcwallet/rpctest"
)
...
func main() {
	...
	fmt.Println("Command for node's abcctl:")
	fmt.Printf("\tabcctl -u %s -P %s -s %s -c %s\n", cn.User, cn.Pass,
		cn.Host, nodeCertFile)

	cw := primaryHarness.RPCWalletConfig()
	walletCertFile := primaryHarness.RPCWalletCertFile()
	fmt.Println("Command for wallet's abcctl:")
	fmt.Printf("\tabcctl -u %s -P %s -s %s -c %s --wallet\n", cw.User, cw.Pass,
