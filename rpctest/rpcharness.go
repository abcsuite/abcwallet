...
import (
	...
	rpc "github.com/abcsuite/abcrpcclient"
...
// Harness fully encapsulates an active abcd process, along with an embedded
// abcwallet to provide a unified platform for creating RPC-driven integration
// tests involving abcd. The active abcd node will typically be run in simnet
...
// spends. The active abcd process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start the abcd node itself. This spawns a new process which will be
	// managed
	...
	// Start abcwallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with abcd's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr abcutil.Address
	for i := 0; i < 100; i++ {
...
// abcd process belonging to this Harness instance. If the initial connection
