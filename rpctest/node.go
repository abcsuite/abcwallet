...
// nodeConfig contains all the args, and data required to launch a abcd process
...
// command returns the exec.Cmd which will be used to start the abcd process.
...
// to connect to the abcd process that is launched via Start().
...
// as the base for the log and data directories for abcd.
...
// Start creates a new abcd process, and writes its pid in a file reserved for
...
// Stop interrupts the running abcd process, and waits until it exits properly.
...
func genCertPair(certFile, keyFile, certFileWallet, keyFileWallet string) error {
	...
	cert, key, err := abcutil.NewTLSCertPair(elliptic.P521(), org, validUntil,
		nil)
	if err != nil {
		return err
	}
