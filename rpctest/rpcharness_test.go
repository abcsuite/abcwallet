...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := abcrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = abcutil.DecodeAddress(addrStr, r.ActiveNet)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := abcutil.DecodeNetworkAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := abcutil.DecodeAddress(devSubAddrStr, &chaincfg.SimNetParams)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*abcjson.RPCError).Code !=
		abcjson.ErrRPCWalletPassphraseIncorrect {
		// abcjson.ErrWalletPassphraseIncorrect.Code
	...
	// abcjson.ErrRPCWalletUnlockNeeded
	...
	if err != nil && err.(*abcjson.RPCError).Code !=
		abcjson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	preAccountBalances := make(map[string]abcjson.GetAccountBalanceResult)
	...
	sendAmount := abcutil.Amount(700000000)
	...
	postAccountBalances := make(map[string]abcjson.GetAccountBalanceResult)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := abcutil.Amount(700000000)
	...
	// Get *abcutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = abcutil.Amount(txIn.ValueIn).ToCoin()
	}
	...
	for txinID, amt := range txInIDs {
		if _, ok := utxosBeforeSend[txinID]; !ok {
			t.Fatalf("Failed to find txid %v (%v AER) in list of UTXOs",
				txinID, amt)
		}
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := abcutil.Amount(1000000)
	...
	fee := abcutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []abcutil.Amount{700000000, 1400000000}
	addresses := []abcutil.Address{}
	...
	addressAmounts := make(map[abcutil.Address]abcutil.Amount)
	totalAmountToSend := abcutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	if *txList1[0].TxType != abcjson.LTTTRegular {
	...
	voutAmt := abcutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := abcutil.Amount(240000000)
	...
	var sendResult, recvResult abcjson.ListTransactionsResult
	...
	rxtxResults := map[string]abcjson.ListTransactionsResult{
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// abcrpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := abcutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := abcutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := abcutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(abcutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// abcrpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := abcutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := abcutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := abcutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := abcutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(abcutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := abcutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := abcutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *abcrpcclient.Client, t *testing.T) *abcjson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *abcutil.Block, []*chainhash.Hash) {
...
func getBestBlock(r *Harness, t *testing.T) (uint32, *abcutil.Block, *chainhash.Hash) {
...
func includesTx(txHash *chainhash.Hash, block *abcutil.Block,
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *abcutil.Block,
	r *Harness, t *testing.T) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *abcutil.Tx) abcutil.Amount {
	var totalSpent int64
	...
	return abcutil.Amount(totalSpent - totalSent)
}

// getOutPointString uses OutPoint.String() to combine the tx hash with vout
// index from a ListUnspentResult.
func getOutPointString(utxo *abcjson.ListUnspentResult) (string, error) {
