...
type TicketPurchaser struct {
	...
	abcdChainSvr     *abcrpcclient.Client
...
func (t *TicketPurchaser) BalanceToMaintain() abcutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = abcutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() abcutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = abcutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() abcutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = abcutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() abcutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = abcutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// TicketAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) TicketAddress() abcutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetTicketAddress(ticketAddress abcutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() abcutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress abcutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	var ticketAddress abcutil.Address
	var err error
	if cfg.TicketAddress != "" {
		ticketAddress, err = abcutil.DecodeAddress(cfg.TicketAddress,
			activeNet)
		if err != nil {
			return nil, err
		}
	}
	var poolAddress abcutil.Address
	if cfg.PoolAddress != "" {
		poolAddress, err = abcutil.DecodeNetworkAddress(cfg.PoolAddress)
		if err != nil {
			return nil, err
		}
	}
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: abcutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            abcutil.Amount(cfg.MaxFee),
		minFee:            abcutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  abcutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  abcutil.Amount
	PriceNext     abcutil.Amount
	PriceCurrent  abcutil.Amount
	Purchased     int
	LeftWindow    int
	Balance       abcutil.Amount
	TicketPrice   abcutil.Amount
}
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		...
		log.Debugf("Transaction relay fee: %v", abcutil.Amount(t.cfg.TxFee))
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfo(t.abcdChainSvr)
			if err != nil {
		...
		t.stakeVoteSubsidy = abcutil.Amount(subsidy)
	...
	if avgPriceAmt < abcutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = abcutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	sDiffEsts, err := t.abcdChainSvr.EstimateStakeDiff(nil)
	if err == nil {
		ps.PriceNext, err = abcutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt abcutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.abcdChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	// have enough moneys.
	notEnough := func(bal abcutil.Amount, toBuy int, sd abcutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := abcutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
