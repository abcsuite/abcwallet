...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (abcutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt abcutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		poolValue, err := t.abcdChainSvr.GetTicketPoolValue()
		if err != nil {
			return 0, err
		}
		bestBlockH, err := t.abcdChainSvr.GetBestBlockHash()
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.abcdChainSvr.GetBlockHeader(bestBlockH)
		if err != nil {
			return 0, err
		}
		...
		avgPricePoolAmt = poolValue / abcutil.Amount(poolSize)
	...
	if t.priceMode == AvgPriceVWAPMode || t.priceMode == AvgPriceDualMode {
		...
		for i := 0; i < vwapReqTries; i++ {
			ticketVWAP, err = t.abcdChainSvr.TicketVWAP(&startVWAPHeight,
				&endVWAPHeight)
