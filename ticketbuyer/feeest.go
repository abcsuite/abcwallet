...
type diffPeriodFee struct {
	difficulty abcutil.Amount
	difference abcutil.Amount // Difference from current difficulty
	fee        abcutil.Amount
}
...
func (t *TicketPurchaser) findClosestFeeWindows(difficulty abcutil.Amount,
	useMedian bool) (abcutil.Amount, error) {
	wtcUint32 := uint32(windowsToConsider)
	info, err := t.abcdChainSvr.TicketFeeInfo(&zeroUint32, &wtcUint32)
	if err != nil {
		return 0.0, err
	}
	...
	for i := range info.FeeInfoWindows {
		...
		blH, err := t.abcdChainSvr.GetBlockHash(startHeight)
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.abcdChainSvr.GetBlockHeader(blH)
		if err != nil {
			return 0, err
		}

		windowDiffAmt := abcutil.Amount(blkHeader.SBits)

		var fee abcutil.Amount
		if !useMedian {
			fee, err = abcutil.NewAmount(info.FeeInfoWindows[i].Mean)
		} else {
			fee, err = abcutil.NewAmount(info.FeeInfoWindows[i].Median)
		}
		if err != nil {
			return 0, err
		}
...
func (t *TicketPurchaser) findTicketFeeBlocks(useMedian bool) (abcutil.Amount, error) {
	btaUint32 := uint32(t.cfg.BlocksToAvg)
	info, err := t.abcdChainSvr.TicketFeeInfo(&btaUint32, nil)
	if err != nil {
		return 0.0, err
	}

	var sum, tmp abcutil.Amount
	for i := range info.FeeInfoBlocks {
		if !useMedian {
			tmp, err = abcutil.NewAmount(info.FeeInfoBlocks[i].Mean)
		} else {
			tmp, err = abcutil.NewAmount(info.FeeInfoBlocks[i].Median)
		}
		if err != nil {
			return 0, err
		}
	...
	return sum / abcutil.Amount(t.cfg.BlocksToAvg), nil
}
