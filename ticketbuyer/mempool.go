...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.ticketAddress != nil {
		tiHashes, err := t.abcdChainSvr.GetRawMempool(abcjson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.abcdChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
			...
			addr, err := abcutil.DecodeNetworkAddress(addrStr)
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.abcdChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
