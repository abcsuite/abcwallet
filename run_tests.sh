...
DOCKER_IMAGE_TAG=aero-golang-builder-$GOVERSION

docker pull aero/$DOCKER_IMAGE_TAG

docker run --rm -it -v $(pwd):/src aero/$DOCKER_IMAGE_TAG /bin/bash -c "\
  rsync -ra --filter=':- .gitignore'  \
  /src/ /go/src/github.com/abcsuite/$REPO/ && \
  cd github.com/abcsuite/$REPO/ && \
  glide install && \
