...
import (
	...
	"github.com/abcsuite/abcwallet/wallet/internal/txsizes"
)

func p2pkhOutputs(amounts ...abcutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	f := func(target abcutil.Amount) (abcutil.Amount, []*wire.TxIn, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			...
			currentTotal += abcutil.Amount(u.Value)
...
func TestNewUnsignedTransaction(t *testing.T) {
	...
	for i, test := range tests {
		...
		if tx.ChangeIndex < 0 {
			...
			changeAmount := abcutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
			if test.ChangeAmount == 0 {
