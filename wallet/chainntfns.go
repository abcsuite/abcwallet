...
func (w *Wallet) handleConsensusRPCNotifications(chainClient *chain.RPCClient) {
	for n := range chainClient.Notifications() {
		...
		case chain.ClientConnected:
			log.Infof("The client has successfully connected to abcd and " +
...
func (w *Wallet) evaluateStakePoolTicket(rec *udb.TxRecord,
	blockHeight int32, poolUser abcutil.Address) (bool, error) {
	tx := rec.MsgTx
	...
	out := abcutil.Amount(0)
	for i := range tx.TxOut {
		out += abcutil.Amount(tx.TxOut[i].Value)
	}
	fees := in - out
	...
	if exists {
		...
		feeNeeded := txrules.StakePoolTicketFee(abcutil.Amount(
