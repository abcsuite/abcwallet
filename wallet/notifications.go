...
func lookupOutputChain(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails,
	cred udb.CreditRecord) (account uint32, internal bool, address abcutil.Address,
...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails) TransactionSummary {
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		for _, txOut := range details.MsgTx.TxOut {
			fee -= abcutil.Amount(txOut.Value)
		}
	}
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]abcutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]abcutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]abcutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *udb.TxDetails) {
	...
	bals := make(map[uint32]abcutil.Amount)
