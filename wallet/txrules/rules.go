...
const DefaultRelayFeePerKb abcutil.Amount = 1e5

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount abcutil.Amount, scriptSize int, relayFeePerKb abcutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb abcutil.Amount) bool {
	...
	return IsDustAmount(abcutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb abcutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > abcutil.MaxAmount {
...
func FeeForSerializeSize(relayFeePerKb abcutil.Amount, txSerializeSize int) abcutil.Amount {
	fee := relayFeePerKb * abcutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > abcutil.MaxAmount {
		fee = abcutil.MaxAmount
	}
