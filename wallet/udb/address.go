...
type ManagedAddress interface {
	...
	// Address returns a abcutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *abcutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the abcutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() abcutil.Address {
	return a.address
}
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *abcutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() abcutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := abcutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
