...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the abcutil.Tx of the read MsgTx and set its index.
		tx := abcutil.NewTx(msgTx)
		tx.SetIndex(abcutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: abcutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
