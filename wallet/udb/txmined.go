...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, abcutil.Address) (uint32, error)
}
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			...
			minedBalance += abcutil.Amount(output.Value)
		}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     abcutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		v := valueUnminedCredit(abcutil.Amount(rec.MsgTx.TxOut[index].Value),
	...
	txOutAmt := abcutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) addMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord,
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		abcutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func approvesParent(voteBits uint16) bool {
	return abcutil.IsFlagSet16(voteBits, abcutil.BlockValid)
}
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	addr abcutil.Address) ([]*MultisigCredit, error) {
...
func (s *Store) unspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	...
	if sha, ok := addr.(*abcutil.AddressScriptHash); ok {
...
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket,
	amt abcutil.Amount, height int32, minConf int32, all bool,
...
type InputSource struct {
	source func(abcutil.Amount) (abcutil.Amount, []*wire.TxIn, [][]byte, error)
}
...
func (s *InputSource) SelectInputs(target abcutil.Amount) (abcutil.Amount, []*wire.TxIn, [][]byte, error) {
...
func (s *Store) MakeInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf, syncHeight int32) InputSource {
	...
	var (
		currentTotal   abcutil.Amount
		currentInputs  []*wire.TxIn
		currentScripts [][]byte
	)

	f := func(target abcutil.Amount) (abcutil.Amount, []*wire.TxIn, [][]byte, error) {
...
func (s *Store) balanceFullScanSimulated(ns walletdb.ReadBucket, minConf int32,
	syncHeight int32, unminedInputs map[string][]byte) (abcutil.Amount, error) {
	if minConf <= 0 {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards abcutil.Amount
	ImmatureStakeGeneration abcutil.Amount
	LockedByTickets         abcutil.Amount
	Spendable               abcutil.Amount
	Total                   abcutil.Amount
	VotingAuthority         abcutil.Amount
}
