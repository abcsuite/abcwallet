...
import (
	...
	walletchain "github.com/abcsuite/abcwallet/chain"
...
func (s *StakeStore) insertSStx(ns walletdb.ReadWriteBucket, sstx *abcutil.Tx) error {
...
func (s *StakeStore) dumpSStxHashesForAddress(ns walletdb.ReadBucket, addr abcutil.Address) ([]chainhash.Hash, error) {
	...
	_, addrIsP2SH := addr.(*abcutil.AddressScriptHash)
...
func (s *StakeStore) sstxAddress(ns walletdb.ReadBucket, hash *chainhash.Hash) (abcutil.Address, error) {
	// Access the database and store the result locally.
	...
	if p2sh {
		addr, err = abcutil.NewAddressScriptHashFromHash(thisHash160, s.Params)
	} else {
		addr, err = abcutil.NewAddressPubKeyHash(thisHash160, s.Params, chainec.ECTypeSecp256k1)
	}
	if err != nil {
		str := "failure getting ticket 0th out script hashes from db"
		return nil, stakeStoreError(apperrors.ErrDatabase, str, err)
...
// it's an SSRtx.
func signVRTransaction(m *Manager, ns walletdb.ReadBucket, msgTx *wire.MsgTx, sstx *abcutil.Tx, isSSGen bool) error {
	txInNumToSign := 0
	...
	var getKey txscript.KeyClosure = func(addr abcutil.Address) (chainec.PrivateKey, bool, error) {
	...
	var getScript txscript.ScriptClosure = func(addr abcutil.Address) ([]byte, error) {
...
func (s *StakeStore) generateRevocation(ns walletdb.ReadWriteBucket, waddrmgrNs walletdb.ReadBucket,
	blockHash *chainhash.Hash, height int64, sstxHash *chainhash.Hash, feePerKb abcutil.Amount,
...
func (s *StakeStore) HandleMissedTicketsNtfn(ns walletdb.ReadWriteBucket, waddrmgrNs walletdb.ReadBucket, blockHash *chainhash.Hash,
	blockHeight int64, tickets []*chainhash.Hash, feePerKb abcutil.Amount, allowHighFees bool) ([]*StakeNotification, error) {
...
func (s *StakeStore) updateStakePoolUserTickets(ns walletdb.ReadWriteBucket, waddrmgrNs walletdb.ReadBucket, user abcutil.Address, ticket *PoolTicket) error {
	_, isScriptHash := user.(*abcutil.AddressScriptHash)
	_, isP2PKH := user.(*abcutil.AddressPubKeyHash)
...
func (s *StakeStore) removeStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user abcutil.Address,
...
// database, it adds it.
func (s *StakeStore) updateStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user abcutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*abcutil.AddressScriptHash)
	_, isP2PKH := user.(*abcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		str := fmt.Sprintf("user %v is invalid", user.EncodeAddress())
...
func stakePoolUserInfo(ns walletdb.ReadBucket, user abcutil.Address) (*StakePoolUser, error) {
	_, isScriptHash := user.(*abcutil.AddressScriptHash)
	_, isP2PKH := user.(*abcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		str := fmt.Sprintf("user %v is invalid", user.EncodeAddress())
...
func (s *StakeStore) StakePoolUserInfo(ns walletdb.ReadBucket, user abcutil.Address) (*StakePoolUser, error) {
