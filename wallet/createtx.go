...
func feeForSize(incr abcutil.Amount, sz int) abcutil.Amount {
	return abcutil.Amount(1+sz/1000) * incr
}
...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb abcutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(abcutil.Amount) (abcutil.Amount, []*wire.TxIn, [][]byte, error) {
				total, inputs, prevScripts, err := sourceImpl.SelectInputs(abcutil.MaxAmount)
				switch err.(type) {
...
func (s *secretSource) GetKey(addr abcutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr abcutil.Address) ([]byte, error) {
...
func (w *Wallet) insertIntoTxMgr(ns walletdb.ReadWriteBucket, msgTx *wire.MsgTx) (*udb.TxRecord, error) {
	...
	if err != nil {
		return nil, abcjson.ErrInternal
	}
...
//
// Aero: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(outputs []*wire.TxOut, account uint32, minconf int32, chainClient *chain.RPCClient,
	randomizeChangeIdx bool, txFee abcutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := abcutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisigInternal(dbtx walletdb.ReadWriteTx, account uint32,
	amount abcutil.Amount, pubkeys []*abcutil.AddressSecpPubKey, nRequired int8,
	minconf int32) (*CreatedTx, abcutil.Address, []byte, error) {
	...
	txToMultisigError :=
		func(err error) (*CreatedTx, abcutil.Address, []byte, error) {
	...
	// fee estimator.
	var feeEstForTx abcutil.Amount
	switch {
	...
	scAddr, err := abcutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(err)
	}
	...
	// Request updates from abcd for new transactions sent to this
	// script hash address.
	utilAddrs := make([]abcutil.Address, 1)
	utilAddrs[0] = scAddr
	err = chainClient.LoadTxFilter(false, []abcutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(err)
	}
...
func (w *Wallet) compressWallet(maxNumIns int, account uint32, changeAddr abcutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr abcutil.Address) (*chainhash.Hash, error) {
...
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint,
	input *extendedOutPoint, addrVote abcutil.Address, addrSubsidy abcutil.Address,
	ticketCost int64, addrPool abcutil.Address) (*wire.MsgTx, error) {
	mtx := wire.NewMsgTx()
	...
	zeroed := [20]byte{}
	addrZeroed, err := abcutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			abcutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		abcutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
...
func (w *Wallet) purchaseTickets(req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(persistReturnedChildFunc, uint32) (abcutil.Address, error) {
			return addr, err
		}
	}
	...
	ticketPrice, err := abcutil.NewAmount(ticketPricesF64.NextStakeDifficulty)
	if err != nil {
		return nil, err
	}
	...
	var neededPerTicket, ticketFee abcutil.Amount
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * abcutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy abcutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       abcutil.Amount(eopPool.amt),
		...
		err = walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
			...
			if _, err := w.Manager.Address(addrmgrNs, addrVote); err == nil {
				if w.ticketAddress == nil {
					err = w.StakeMgr.InsertSStx(stakemgrNs, abcutil.NewTx(ticket))
					if err != nil {
						return err
					}
				}
...
func (w *Wallet) txToSStx(pair map[string]abcutil.Amount,
	inputCredits []udb.Credit, inputs []abcjson.SStxInput,
	payouts []abcjson.SStxCommitOut, account uint32, minconf int32) (*CreatedTx, error) {
...
func (w *Wallet) txToSStxInternal(dbtx walletdb.ReadWriteTx, pair map[string]abcutil.Amount,
	inputCredits []udb.Credit, inputs []abcjson.SStxInput,
	payouts []abcjson.SStxCommitOut, account uint32, minconf int32) (tx *CreatedTx, err error) {
	...
	for addrStr, amt := range pair {
		...
		minAmount += amt
		addr, err := abcutil.DecodeAddress(addrStr, w.chainParams)
		if err != nil {
	...
	for _, input := range inputs {
		...
		if err != nil {
			return nil, abcjson.ErrDecodeHexString
		}

		if input.Vout < 0 {
			return nil, abcjson.Error{
				Code:    abcjson.ErrInvalidParameter.Code,
		...
		totalAdded += abcutil.Amount(input.Amt)
	}
	...
	for i := range inputs {
		...
		if payouts[i].Addr == "" {
			...
			addr, err = abcutil.DecodeAddress(payouts[i].Addr,
				w.chainParams)
			if err != nil {
				return nil, fmt.Errorf("cannot decode address: %s", err)
			}
			...
			switch addr.(type) {
			case *abcutil.AddressPubKeyHash:
			default:
				return nil, abcjson.ErrInvalidAddressOrKey
			}
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			abcutil.Amount(payouts[i].CommitAmt), limits)
		if err != nil {
			return nil, fmt.Errorf("cannot create txout script: %s", err)
		}
		txout := wire.NewTxOut(int64(0), pkScript)
		...
		if payouts[i].ChangeAddr == "" {
			...
			a, err := abcutil.DecodeAddress(payouts[i].ChangeAddr, w.chainParams)
			if err != nil {
				return nil, err
			}
			// Ensure the address is one of the supported types and that
			...
			switch a.(type) {
			case *abcutil.AddressPubKeyHash:
			case *abcutil.AddressScriptHash:
			default:
				return nil, abcjson.ErrInvalidAddressOrKey
			}
			changeAddr = a
		...
		err = addSStxChange(msgtx,
			abcutil.Amount(payouts[i].ChangeAmt),
			changeAddr)
		if err != nil {
			return nil, err
		}
...
func addSStxChange(msgtx *wire.MsgTx, change abcutil.Amount,
	changeAddr abcutil.Address) error {
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount abcutil.Amount, currentHeight int32) ([]udb.Credit, error) {
