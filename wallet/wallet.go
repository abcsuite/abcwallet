...
import (
	...
	"github.com/abcsuite/abcd/abcec/secp256k1"
	...
	"github.com/abcsuite/abcwallet/wallet/txauthor"
...
type Wallet struct {
	...
	balanceToMaintain       abcutil.Amount
	poolAddress             abcutil.Address
	poolFees                float64
	...
	relayFee               abcutil.Amount
	relayFeeMu             sync.Mutex
	ticketFeeIncrementLock sync.Mutex
	ticketFeeIncrement     abcutil.Amount
	DisallowFree           bool
	AllowHighFees          bool
...
func newWallet(votingEnabled bool, addressReuse bool, ticketAddress abcutil.Address,
	poolAddress abcutil.Address, pf float64, relayFee, ticketFee abcutil.Amount,
...
func (w *Wallet) StakeDifficulty() (abcutil.Amount, error) {
	...
	sd, err := abcutil.NewAmount(sdResp.NextStakeDifficulty)
	if err != nil {
		return 0, err
	}
...
func (w *Wallet) BalanceToMaintain() abcutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance abcutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() abcutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee abcutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() abcutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee abcutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) loadActiveAddrs(dbtx walletdb.ReadTx, chainClient *chain.RPCClient) (uint64, error) {
	pool := sync.Pool{New: func() interface{} { return make([]abcutil.Address, 0, 256) }}
	recycleAddrs := func(addrs []abcutil.Address) { pool.Put(addrs[:0]) }
	getAddrs := func() []abcutil.Address { return pool.Get().([]abcutil.Address) }
...
func (w *Wallet) FetchHeaders(chainClient *chain.RPCClient) (count int, rescanFrom chainhash.Hash, rescanFromHeight int32,
	...
	//
	// See https://github.com/abcsuite/abcd/issues/427 for details.  This hack
...
func (w *Wallet) syncWithChain(chainClient *chain.RPCClient) error {
	...
	// startup.
	// TODO A proper pass through for abcrpcclient for these cmds.
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*abcutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
	...
	createSStxRequest struct {
		...
		pair       map[string]abcutil.Amount
		couts      []abcjson.SStxCommitOut
		inputs     []abcjson.SStxInput
		minconf    int32
	...
	purchaseTicketRequest struct {
		minBalance  abcutil.Amount
		spendLimit  abcutil.Amount
		minConf     int32
		ticketAddr  abcutil.Address
		account     uint32
		numTickets  int
		poolAddress abcutil.Address
		poolFees    float64
		expiry      int32
		txFee       abcutil.Amount
		ticketFee   abcutil.Amount
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address abcutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount abcutil.Amount,
	pubkeys []*abcutil.AddressSecpPubKey, nrequired int8,
	minconf int32) (*CreatedTx, abcutil.Address, []byte, error) {
...
// generate a new SStx.
func (w *Wallet) CreateSStxTx(pair map[string]abcutil.Amount,
	usedInputs []udb.Credit,
	inputs []abcjson.SStxInput,
	couts []abcjson.SStxCommitOut,
...
// tickets.
func (w *Wallet) PurchaseTickets(minBalance, spendLimit abcutil.Amount,
	minConf int32, ticketAddr abcutil.Address, account uint32,
	numTickets int, poolAddress abcutil.Address, poolFees float64,
	expiry int32, txFee abcutil.Amount, ticketFee abcutil.Amount) ([]*chainhash.Hash,
	error) {
...
// spending to it in the blockchain or abcd mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (abcutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a abcutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr abcutil.Address) (sig []byte, err error) {
...
func (w *Wallet) existsAddressOnChain(address abcutil.Address) (bool, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return false, err
	}
...
func (w *Wallet) HaveAddress(a abcutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a abcutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a abcutil.Address) (udb.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager,
	syncHeight int32, net *chaincfg.Params) []abcjson.ListTransactionsResult {
	...
	results := []abcjson.ListTransactionsResult{}
	...
	txTypeStr := abcjson.LTTTRegular
	switch details.TxType {
	case stake.TxTypeSStx:
		txTypeStr = abcjson.LTTTTicket
	case stake.TxTypeSSGen:
		txTypeStr = abcjson.LTTTVote
	case stake.TxTypeSSRtx:
		txTypeStr = abcjson.LTTTRevocation
	}

	// Fee can only be determined if every input is a debit.
	var feeF64 float64
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal abcutil.Amount
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := abcutil.Amount(output.Value).ToCoin()
		result := abcjson.ListTransactionsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   BlockIndex
			//
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]abcjson.ListTransactionsResult, error) {
	txList := []abcjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]abcjson.ListTransactionsResult, error) {
	txList := []abcjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]abcjson.ListTransactionsResult, error) {
	txList := []abcjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*abcutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]abcjson.ListTransactionsResult, error) {
	txList := []abcjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) GetTransactions(startBlock, endBlock *BlockIdentifier, cancel <-chan struct{}) (*GetTransactionsResult, error) {
	...
	var startResp, endResp abcrpcclient.FutureGetBlockVerboseResult
	if startBlock != nil {
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*abcutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*abcjson.ListUnspentResult, error) {
	var results []*abcjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		for i := range unspent {
			...
			case stake.TxTypeSStx:
				...
				// ticket maturity + 1??? Check this AERO TODO
				if output.Index == 0 {
		...
		scSwitch:
			...
			result := &abcjson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr abcutil.Address) (string, error) {
	var privKey chainec.PrivateKey
	var done func()
	defer func() {
		if done != nil {
	...
	wif, err := abcutil.NewWIF(privKey, w.chainParams, privKey.GetType())
	if err != nil {
		return "", err
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *abcutil.WIF) (string, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return "", err
	}
...
func (w *Wallet) ImportScript(rs []byte) error {
	...
	return walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		err = chainClient.LoadTxFilter(false,
			[]abcutil.Address{mscriptaddr.Address()}, nil)
		if err != nil {
			return fmt.Errorf("Failed to subscribe for address ntfns for "+
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr abcutil.Address) ([]byte, error) {
	var scriptCopy []byte
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		ns := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) StakeInfo(chainClient *abcrpcclient.Client) (*StakeInfoData, error) {
	var resp *StakeInfoData
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		stakemgrNs := tx.ReadBucket(wstakemgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)

		mempoolTicketsFuture := chainClient.GetRawMempoolAsync(abcjson.GRMTickets)
		missedTicketsFuture := chainClient.MissedTicketsAsync()

		var (
			poolSize                                         uint32
			ticketHashPtrs, voteHashPtrs, revocationHashPtrs []*chainhash.Hash
			liveTicketsFuture                                abcrpcclient.FutureExistsLiveTicketsResult
			expiredTicketsFuture                             abcrpcclient.FutureExistsExpiredTicketsResult
		...
		go func() {
			...
			err1 = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
				...
				for _, voteHash := range voteHashPtrs {
					...
					totalSubsidy += abcutil.Amount(msgTx.TxIn[0].ValueIn)
				}
				return nil
			})
			wg.Done()
...
func (w *Wallet) LockedOutpoints() []abcjson.TransactionInput {
	locked := make([]abcjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = abcjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr abcutil.Address) error {
...
// returning the total amount of aero received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr abcutil.Address, minConf int32) (abcutil.Amount, error) {
	var amount abcutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*abcutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr abcutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr abcutil.Address) ([]byte, error) {
...
func Open(db walletdb.DB, pubPass []byte, votingEnabled bool, addressReuse bool,
	...
	if ticketAddress != "" {
		ticketAddr, err = abcutil.DecodeAddress(ticketAddress, params)
		if err != nil {
	...
	if poolAddress != "" {
		poolAddr, err = abcutil.DecodeAddress(poolAddress, params)
		if err != nil {
	...
	ticketFeeAmt, err := abcutil.NewAmount(ticketFee)
	if err != nil {
		return nil, err
	}

	relayFeeAmt, err := abcutil.NewAmount(relayFee)
	if err != nil {
		return nil, err
	}
