...
// RPCClient represents a persistent client connection to a aero RPC server
// for information regarding the current best block chain.
type RPCClient struct {
	*abcrpcclient.Client
	connConfig        *abcrpcclient.ConnConfig // Work around unexported field
...
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	...
	client := &RPCClient{
		connConfig: &abcrpcclient.ConnConfig{
			Host:                 connect,
			Endpoint:             "ws",
			User:                 user,
			Pass:                 pass,
	...
	ntfnCallbacks := &abcrpcclient.NotificationHandlers{
	...
	rpcClient, err := abcrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
func (c *RPCClient) Start() (err error) {
	...
	if err == nil {
		versionResult := versions["abcdjsonrpcapi"]
		serverAPI = semver{
...
// abcrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// aero RPC server.  This channel must be continually read or the process
...
// POSTClient creates the equivalent HTTP POST abcrpcclient.Client.
func (c *RPCClient) POSTClient() (*abcrpcclient.Client, error) {
	configCopy := *c.connConfig
	configCopy.HTTPPostMode = true
	return abcrpcclient.New(&configCopy, nil)
}
