...
var certificateFile = filepath.Join(abcutil.AppDataDir("abcwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", abcutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/abcwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/abcwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by abcwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.abcwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\abcwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by abcwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "abcwallet");
...
if (os.platform() == 'win32') {
  certPath = path.join(process.env.LOCALAPPDATA, 'abcwallet', 'rpc.cert');
} else if (os.platform() == 'darwin') {
  certPath = path.join(process.env.HOME, 'Library', 'Application Support',
    'abcwallet', 'rpc.cert');
} else {
  certPath = path.join(process.env.HOME, '.abcwallet', 'rpc.cert');
}
...
```bash
$ protoc -I /path/to/abcsuite/abcwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "abcwallet", "rpc.cert")
    elif platform.system() == 'Darwin':
        cert_file_path = os.path.join(os.environ['HOME'], 'Library', 'Application Support',
                                      'abcwallet', 'rpc.cert')
    else:
        cert_file_path = os.path.join(os.environ['HOME'], '.abcwallet', 'rpc.cert')
