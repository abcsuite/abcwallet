...
the management of the wallet and its connection to the Aero network.  It has
...
dynamically start the abcd RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the Aero network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default abcd RPC port of the
  active Aero network.
...
- `string public_key`: The public key encoded as a string in the Aero encoding 
  format.
...
the Aero network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of abcd. Right now, wallet block processing 
