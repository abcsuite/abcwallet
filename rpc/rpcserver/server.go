...
//
//   https://github.com/abcsuite/abcwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/abcsuite/abcwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/abcsuite/abcwallet/walletseed"
)
...
// abcutil.DecodeAddress, which does not perform the network check.
func decodeAddress(a string, params *chaincfg.Params) (abcutil.Address, error) {
	addr, err := abcutil.DecodeAddress(a, params)
	if err != nil {
...
// as well as establishing a RPC connection to a abcd consensus server.
...
func (s *walletServer) NextAddress(ctx context.Context, req *pb.NextAddressRequest) (
	*pb.NextAddressResponse, error) {
	...
	pubKeyAddr, err := abcutil.NewAddressSecpPubKey(pubKey.Serialize(), s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := abcutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
...
func (s *walletServer) ImportScript(ctx context.Context,
	...
	p2sh, err := abcutil.NewAddressScriptHash(req.Script, s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	totalAmount, inputs, scripts, err := s.wallet.SelectInputs(abcutil.Amount(req.TargetAmount), policy)
	...
	if req.IncludeChangeScript && totalAmount > abcutil.Amount(req.TargetAmount) {
...
func (s *walletServer) ConstructTransaction(ctx context.Context, req *pb.ConstructTransactionRequest) (
	*pb.ConstructTransactionResponse, error) {
	...
	if req.FeePerKb != 0 {
		feePerKb = abcutil.Amount(req.FeePerKb)
	}
...
func (s *walletServer) PurchaseTickets(ctx context.Context,
	...
	// purchase request.
	spendLimit := abcutil.Amount(req.SpendLimit)
	if spendLimit < 0 {
		return nil, status.Errorf(codes.InvalidArgument,
	...
	txFee := abcutil.Amount(req.TxFee)
	ticketFee := abcutil.Amount(req.TicketFee)
...
func (s *loaderServer) StartConsensusRpc(ctx context.Context, req *pb.StartConsensusRpcRequest) (
	*pb.StartConsensusRpcResponse, error) {
	...
	err = rpcClient.Start()
	if err != nil {
		if err == abcrpcclient.ErrInvalidAuth {
			return nil, status.Errorf(codes.InvalidArgument,
