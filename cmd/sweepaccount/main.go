...
var (
	walletDataDirectory = abcutil.AppDataDir("abcwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool                `long:"testnet" description:"Use the test aero network"`
	SimNet                bool                `long:"simnet" description:"Use the simulation aero network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple aero networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []abcjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue abcutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := abcutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(abcutil.Amount) (abcutil.Amount, []*wire.TxIn, [][]byte, error) {
...
// function.
func makeDestinationScriptSource(rpcClient *abcrpcclient.Client, accountName string) txauthor.ChangeSource {
...
func sweep() error {
	...
	rpcClient, err := abcrpcclient.New(&abcrpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]abcjson.ListUnspentResult)
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := abcutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount abcutil.Amount) bool {
	return amount >= 0 && amount <= abcutil.MaxAmount
}

func parseOutPoint(input *abcjson.ListUnspentResult) (wire.OutPoint, error) {
